...
function DialogItem({ dialog }: { dialog: DialogConfig }): React.ReactElement {
  ...
  if (dialog.type === 'component') {
    if (React.isValidElement(dialog.component)) {
      // React 19에서 element.props 타입이 unknown으로 변경됨
      const props = dialog.component.props as Record<string, unknown>;
      const extraProps =
        'showBackground' in props ? { showBackground: dialog.showBackground } : {};
